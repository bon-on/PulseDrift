# PulseDrift PWA GitHub Pages Verification
...
- `flutter build web --release --base-href /PulseDrift/`: passed, built `build/web`.
...
```html
<base href="/PulseDrift/">
```

`build/web/manifest.json` contains `PulseDrift` for both `name` and `short_name`.
